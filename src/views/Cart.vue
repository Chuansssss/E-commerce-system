<template>
  <div>
    ...
    <Modal :mdShow="modalConfirm" @close="closeModal">
      <p slot="message">R u sure u want to delete?</p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delCart">Confirm</a>
        <a class="btn btn--m" href="javascript:;" @click="modalConfirm=false">Cancel</a>
      </div>
    </Modal>
...
<script>
  ...
  export default{
      ...
      computed: { //computed variable don't need to be declared ahead
        checkAllFlag() { // real-time computed value cannot be assigned later
      ...
      methods: {
          ...
          delCart() {
            axios.post("/users/delItem", {
                ...
                if (res.status == '0') {
                    ...
                    this.init();
                    var productNum = res.result;
                    this.$store.commit("updateCartCount", this.$store.state.cartCount - productNum);
                }
            })
          },
          closeModal() {
          ...
          editCart(flag, item) {
              ...
              axios.post('/users/editCart', {
                  ...
                  this.$store.commit("updateCartCount", this.$store.state.cartCount + num);
              })
          },
          toggleCheckAll() {
